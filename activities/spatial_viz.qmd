...
ggplot(energy, aes(y = therms, x = renovated)) + 
  geom_boxplot() + 
  facet_wrap(~ address)
# A density plot isn't that  helpful for comparing typical therms in this example!
...
```{r}
# Now we create an interactive map. 
leaflet(data = fave_places) %>% 
  addTiles()
```

```{r}
# Now we add markers to the map based on the longitude and latitude columns in the fave_places dataset.
...
leaflet(data = fave_places) %>% 
  addTiles() %>% 
  addCircles(color = col2hex("darkorange"))
```
...
leaflet(data = fave_places) %>%
  ...
  addCircles(weight = 10, opacity = 1, color = col2hex('red')) %>%
  addPolylines(
    lng = ~longitude,
    lat = ~latitude,
    color = col2hex("green")
...
```{r}
ggplot(starbucks, aes(y=Latitude, x=Longitude)) + geom_point(size = 0.3)
```
...
library(mosaic): Loads the mosaic package, which simplifies data analysis and visualization.

ggplot(world_boundaries): Initializes a ggplot using the world_boundaries dataset (containing country boundaries).

geom_sf(): Plots the country boundaries using the simple feature format (spatial data format).

geom_point(...): Adds red points for each Starbucks location from the starbucks dataset, using Longitude and Latitude to place them on the map.

data = starbucks: Specifies that Starbucks data is used for plotting points.
aes(x = Longitude, y = Latitude): Maps the Starbucks longitude to the x-axis and latitude to the y-axis.
alpha = 0.3, size = 0.2, color = "red": Adjusts the transparency (alpha), size, and color of the points.
theme_map(): Applies a minimalist map theme to make the plot look like a clean map.
...
```{r}
# Get the country boundaries for Canada, Mexico, and the US
north_america <- ne_countries(country = c('Canada', 'United States of America', 'Mexico'), returnclass = 'sf')

# Plot the map and add Starbucks locations
ggplot(north_america) + 
  geom_sf() + 
  geom_point(
    data = starbucks,  # Assuming starbucks dataset has the locations
    aes(x = Longitude, y = Latitude),
    alpha = 0.3, size = 0.2, color = "red"
  ) +
  coord_sf(xlim = c(-170, -50), ylim = c(10, 80)) +  # Zooming in on North America
  theme_map()
```

```{r}
# getting one for South Asia

south_asia <- ne_countries(country = c('Afghanistan', 'Pakistan', 'India'), returnclass = 'sf')

# Plot the map and add Starbucks locations
ggplot(south_asia) + 
  geom_sf() + 
  geom_point(
    data = starbucks,  # Assuming starbucks dataset has locations for South Asia
    aes(x = Longitude, y = Latitude),
    alpha = 0.3, size = 0.2, color = "red"
  ) +
  coord_sf(xlim = c(60, 100), ylim = c(5, 40)) +  # Zooming in on South Asia
  theme_map()
```
...
```{r}
# county-level maps of `median_rent` and `median_age`.
ggplot(elections_by_counties, aes(map_id = county_fips, fill = median_rent)) +
  ...
  theme(legend.position = "right") + 
  coord_equal() + 
  scale_fill_gradientn(name = "median rent", colors = c("white", "orange", "red"))
```
