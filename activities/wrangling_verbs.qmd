...
elections |> select(state_name, county_name, dem_pct_20, dem_pct_16, repub_pct_20, repub_pct_16) |>
  filter(state_name %in% c("Minnesota", "Florida")) 
# if it is one state, only need to use (==) but if it is more than one (%in%). 
```
...
```{r }
# sometimes the order don't make a difference. 
MN1 <- elections |> 
  select(state_name, county_name, dem_pct_20, dem_pct_16) |> 
  filter(state_name == "Minnesota")
