...
```{r}
#Add a density plot for quantitive 
...
ggplot(weather, aes(x = temp3pm, y = location)) + 
  ...
  labs(
    ...
    y = "Location",
    title = "Temperature Distributions by Location at 3 PM"
  ) +
  theme_minimal()
...
-  the temp3pm differe significantly among cities.
- Hobart: Cooler, with temperatures mostly around 15-20°C.
- Uluru: Hotter and variable, ranging from 15-40°C.
- Wollongong: Consistent around 20°C.
...
# fill the raintoday with the probablity of raintomorrow
```
...
#geom_bar(position = "dodge") creates a bar chart where the bars for different groups are placed side by side (horizontally dodged) instead of stacked on top of each other. This makes it easier to compare the categories within each group.
```
...
# you can also try another way too. 
...
# divide based on (yes, no and NA) and than write the location on top of it. 
```
...
ggplot(weather, aes(y = temp3pm, x = date, color = location)) + 
  geom_line() +
  geom_smooth(color="black", size = 0.1) # should come after the "facet_wrap(~location) " so it apply the line for each graph.
...
ggplot(weather, aes(y = temp3pm, x = date)) + 
  geom_line(color= "red") +
  facet_wrap(~location) +
  geom_smooth(color="black", size = 0.5) # should come after the "facet_wrap(~location) " so it apply the line for each graph.
...
There is a strong, negative relationship between noth amoung of spending and salary with sat outcomes. 
The graphs show that as both kind of spending increase, the SAT score outcomes decrease. 
This is surpise find becuase it is counter intuitive. Mostly the logic is that more spending, better SAT score outcome.
...
```{r}
# one way 
ggplot(education, aes(x= salary, color = expend , y=sat))+
...
```{r}
#### Part c {-}
# Make a trivariate visualization that demonstrates the relationship of `sat` with `expend` AND `fracCat`. Highlight the differences in `fracCat` groups through color AND unique trend lines. 
ggplot(education, aes(y = sat, x = expend, color = fracCat)) + 
  geom_point() + 
  geom_smooth(se = FALSE, method = "lm")
```
# What story does your graphic tell? Does it still seem that SAT scores decrease as spending increases?

The graphic shows different trends between SAT scores and spending across categories (fracCat).
In the lowest fraction group (red), SAT scores are higher with a slight positive trend as spending increases.
In the middle group (green), there's a stronger upward trend.
In the highest fraction group (blue), SAT scores are lower, but still show a slight positive trend with increased spending.
...
When looking at the data without considering group categories (fracCat), SAT scores appeared to decrease as spending increased. This was due to a confounding factor: the proportion of disadvantaged students (fracCat) in each group.

When controlling for fracCat, the relationship between SAT scores and spending changes direction: within each category, higher spending actually correlates with higher SAT scores.
