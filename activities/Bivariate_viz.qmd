...
ggplot(survey, aes(x=hangout)) +
  geom_bar(fill = "green")+
  labs (x = "Hung out Location", y = "Number of times")
...
ggplot(survey, aes(x= fave_temp))+
  ...
  labs (x = "Favorate Tempretue Outside", y = "Frequency", title = "Preferance of Favorte temepreute by by students on campus")+
  theme(
...
```{r}
ggplot(weather, aes(x = temp_3pm, fill = location)) +
      geom_density()
```
...
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
  geom_point( color = "blue", shape = "diamond",)
```
...
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
  geom_text(aes(label = state_abbr), color = "red")
```
...
-  **There is strong positive relationship. The highter repub support in 2016, the higher in 2020. TX and UT are the outliers higher in 2020 than 2016. 
...
# There is a strong, negative relationship between both of them.  The higher the the rent, the lower the Repub support tend to be.
```
...
# There is strong, positive relationship, the older the aveerage afein the a country, the higher the repub support.
```
...
ggplot(elections, aes(y = repub_pct_20, x = historical)) +
  geom_point(color = "red", shape = "circle", size = 1)
```
...
ggplot(elections, aes(y = repub_pct_20, x = historical, fill = historical)) +
  geom_violin()+
...
Answer: there is range of country support for Repub within blue, purble and red. However, the Rebub support is higher in red states, midum in purple and lower in blue states. 
...
```{r }
# Two "bad" things about this plot: 
# First:It overlaps and you can't see the blue and purple states clear, only red states. 
# Second: The colors don't match
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density()
```

```{r }
# scale_fill_manual changes the color of the each variable manually as we desire
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
  scale_fill_manual(values = c("blue", "purple", "red"))
```
...
```{r }
# What does alpha = 0.5 do?  It try to provide some transparancy so we can see throught the overlap
# Play around with different values of alpha, between 0 and 1 - The lower the number, the lighter the color and more transparency. The opposite for the higher number, the colors become strong and hard to see. 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
...
```{r }
# facet_wrap splits the plot into separate panels for each category of historical. It solve the issue with overlaping 
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
  scale_fill_manual(values = c("blue", "purple", "red")) +
...
# First, Overlapping Bars: Make it hard to read and interpert catagories. 
# second, the number don't show correctly
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
...
-   Which do you like best? I liked violins and density
-   What is one pro of density plots relative to boxplots? Pro of density plots: They show the shape of the data distribution more clearly, including peaks and valleys.
-   What is one con of density plots relative to boxplots?  Con of density plots: They can be harder to interpret precisely, especially when comparing medians and outliers that are more evident in boxplots
...
```{r }
# A stacked bar plot
# How are the "historical" and "winner_20" variables mapped to the plot, i.e. what roles do they play? winner_20: Fills the bars, showing the proportion of dem and repub votes within each historical group.
ggplot(elections, aes(x = historical, fill = winner_20)) +
  geom_bar()
```
...
# geom_bar(position = "dodge") places the bars side by side instead of stacking them. This makes it easier to compare the counts of winner_20 (e.g., dem vs. repub) within each historical category directly, without overlap.
```
...
# position = "fill": This makes the bars display the proportion of each category (dem and repub) rather than the absolute count. Each bar is scaled to 100%, allowing you to see the relative proportions within each historical group (blue, purple, red).
```
...
pro = easier to compare the relative outcomes in blue vs purple vs red states 
con = lose track of how many counties fall into blue vs purple vs red states
...
```{r}
# this is too many layer and hard to interpret. 
...
# THINK: What DON'T you like about this? Frist, It overlops again in some areas. Second, it is nor organized in any way. 
...
ggplot(elections_by_state, aes(x = repub_pct_20, y = fct_reorder(state_abbr, repub_pct_20))) + 
  geom_point(color = "red") +
   labs(
    title = "Republican Vote Percentage by State (2020)",
    x = "Republican Vote Percentage in 2020",
    y = "State"
  )
```
...
-   What do you think this plot needs? Try it! You might need to do some digging online. It need title and lebles. 
...
- States with the most change: Some states show significant changes in Republican support from 2016 to 2020, with visible shifts in the data. For instance, Arizona and Georgia seem to have decreased their Republican support, reflecting broader electoral changes.

- States with the least change: Republican-leaning states like Alabama and Wyoming show minimal change between 2016 and 2020, maintaining strong Republican support.

- Republican support increase: There may have been slight increases in some traditionally red states or areas where the GOP built upon their base, though the plot doesn’t directly indicate growth areas.

- Republican support decrease: States like Arizona, Georgia, and parts of the Midwest show a notable decrease in Republican support, as they flipped or leaned more toward Democratic candidates in 2020.
