...
-  ggplot(hikes, aes(x= rating)), it initialize ggplot object and the data set is hike. 
-  It sets aesthetic mapping (aes), set the x-axis to be rating 
-  It is missing the geometry (like geom_bar, historgram and point). It means that it set a plot but nothing will be drawn. 
...
ggplot(hikes, aes(x=rating)) + geom_bar(fill= "green") + labs (x= "Rating", y = "Number of Hikes")
```


```{r}
# This time, lets change the code in way that it use different colors for each rating catagory. 
ggplot(hikes, aes(x=rating, fill = rating )) + geom_bar() + labs (x= "Rating", y = "Number of Hikes")
```
...
ggplot(hikes, aes(x= rating, fill = rating)) + geom_bar(color = "red") + labs (x= "Rating", y = "Number of Hikes")
```

```{r}
# Theme was added -(theme_minimal()  minimalistic theme with no background annotations.
ggplot(hikes, aes(x = rating)) +
  geom_bar(colour = "orange", fill = "blue")  +
  labs(x = "Rating", y = "Number of hikes") +
  theme_minimal()
```
```{r}
# In this version, we theme and element_rect to change the backfround of the whole graph to new color or light blue. 
ggplot(hikes, aes(x = rating)) +
  geom_bar(fill = "blue")  +
  labs(x = "Rating", y = "Number of hikes") +
...
ggplot(hikes, aes(x = elevation))+
  geom_bar()
```
This is not an effective because for continuous data, bar chart is not the best option becuse it dose not show pattern, trends and distribution. Bar chart is good for catagorical data. 
...
```{r }
# adding a histogrm layer here. 
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(binwidth = 50, fill = "darkorange", color = "red") 
...
```{r }
# well in this one, I tried to a lot of things to play around with code, title, change background and lables.
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(binwidth = 100, fill = "red") +  
    labs(x= "Elevation (feet)", y= "Number of Hikes", title = "Number of Hikes Based on Elevation") +
  theme(plot.title = element_text(hjust = 0.5, vjust = 1, size = 13, face = "bold", color = "blue"), 
        panel.background = element_rect(fill = "khaki"), plot.background = element_rect(fill = "lightgreen"))
```
...
```{r }
# Adding blue color for the filling of the graph and red for outer layer
ggplot(hikes, aes(x = elevation)) +
...
-   Why does the histogram become ineffective if the `binwidth` is too big (e.g. 1000 feet)?  It put too many hikes together and lose track of the nuances and details
-   Why does the histogram become ineffective if the `binwidth` is too small (e.g. 5 feet)?  we don’t lump enough hikes together and lose track of the bigger picture trends and patterns. We get lost in details. 
...
# chaning the color to red and outer color to blue. 
ggplot(hikes, aes(x = elevation)) +
...
-   INTUITION CHECK: Before tweaking the code and thinking back to `geom_bar()` and `geom_histogram()`, how do you *anticipate* the following code will change the plot?
    ...
    -   `geom_density(color = "blue")`.  color changes the outer layer
    -   `geom_density(fill = "orange")`.   fill changes the inside. 

-   My intuition was correct, that works accross the other graphs. 
...
ggplot(hikes, aes (x = elevation, y = difficulty)) +
  ...
  labs(x = "Elevation", y = "Difficulty", title = "Hike difficulty as the evalivation incease")
```

-   Though not *necessary* to the code working, it's common, good practice to *indent* or *tab* the lines of code after the first line (counterexample below). Why?

- **Answer:** It is important to clarify the sublines or part of the code for clear reading. 
...
# YUCK 
ggplot(hikes, aes(x = elevation)) + 
  geom_histogram(color = "black", fill = "orange", binwidth = 200) + 
  labs(x = "Elevation (feet)", y = "Number of hikes", title = "Number of Hikes Based on Elevation")+
